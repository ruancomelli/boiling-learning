...
class ModelArchitecture:
    ...
    def from_config(cls, config: dict[str, Any]) -> ModelArchitecture:
        return cls(
            tf.keras.models.model_from_json(
                _json.dumps(config), custom_objects=_CUSTOM_OBJECTS
            )
        )
    ...
    def count_parameters(
        self, *, trainable: bool = True, non_trainable: bool = True
    ) -> int:
        if trainable and non_trainable:
            ...
            return int(
                sum(
                    tf.keras.backend.count_params(p)
                    for p in self.model.non_trainable_weights
                )
            )
        else:
            raise ValueError(
                "at least one of `trainable` and `non_trainable` must be true"
            )
...
def _deserialize_model(path: Path, _metadata: Metadata) -> ModelArchitecture:
    ...
    architecture = ModelArchitecture.from_config(
        _json.loads(model_json_path.read_text())
    )
...
def _model_memory_usage_in_bytes(
    ...
    Args:
        architecture: A Keras model architecture.
    ...
    for layer in model.layers:
        if isinstance(layer, tf.keras.Model):
            internal_model_mem_count += _model_memory_usage_in_bytes(
    ...
    *,
    ...
    with strategy_scope(strategy):
        ...
        for layer in new_model.layers:
            layer.set_weights(
                model.model.get_layer(new_to_old[layer.name]).get_weights()
            )
...
def anonymize_model_json(model_json: dict[str, Any]) -> dict[str, Any]:
    ...
    translator = {
        name: f"layer_{index}" for index, name in enumerate(mit.unique_everseen(names))
    }
...
def _rename_layers(config: _Any, translator: Mapping[str, str]) -> _Any:
    ...
    elif isinstance(config, str) and config in translator:

...
output_dir_pattern = str(
    python_project_home_path
    / 'experiments'
    / r'Experiment %Y-%m-%d %H-%M{optional_index}'
)
...
def format_output_dir(output_dir_pattern):
    ...
    def format_time(dirpattern, counter):
        return datetime.now().strftime(
            dirpattern.format(
                index=counter, optional_index=optional_index_format(counter)
            )
        )
    ...
    counter = 0
    if any(
        key in output_dir_pattern for key in ('{index}', '{optional_index}')
    ):
...
with open(filepath, 'w', newline='') as output_file, nidaqmx.Task(
    ...
    while first or should_continue:
        ...
        print_if_must(
            ('anything', 'elapsed time'), f'Elapsed time [s]: {elapsed_time}'
        )
        ...
        if measure_loop_time:
            ...
            if first:
                ...
                if elapsed_time.size > 1:
                    ...
                    loop_time = (
                        elapsed_time
                        - np.c_[previous_elapsed_time[-1], elapsed_time[:-1]]
                    )
                else:
        ...
        if save:
            ...
            if iter_count % write_period == 0:
                print_if_must(
                    ('anything', 'writing'), '>> Writing to file...', end=''
                )

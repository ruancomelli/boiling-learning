...
manager = ModelManager(
    models_path=Path(__file__).parent / 'models', file_name_fmt='{index}.model'
)
...
class Model(BaseEstimator):
    ...
    def __init__(
        self, lamb=0, solver='ne', lr=1, maxiter=1000, tol=1e-5, div_tol=1e6
    ):
        # Initialization
        self.lamb = lamb
        self.solver = solver
    ...
    def grad_J(self, X, y):
        ...
        m = y.size
        return 2 / m * (X.T @ (X @ self.w - y) + self.lamb * self.w)

    def hessian_J(self, X, y=None):
        m = y.size
        n = X.shape[1]
        return 2 / m * (X.T @ X + self.lamb * np.eye(n))

    def cond(self, X, y=None):
        return np.linalg.cond(self.hessian_J(X, y))
    ...
    def __fit_gd(self, X, y):
        ...
        for iter_count in range(self.maxiter):
            ...
            if norm_w > self.div_tol:
                ...
                self.logger[
                    'exit_message'
                ] = f'Divergence found with np.linalg.norm(w) == {norm_w}'
                break

            grad_J = self.grad_J(X_transformed, y)

            self.J_history[iter_count] = (
                mse(self, X, y) + self.lamb / y.size * norm_w ** 2
            )
            ...
            if np.linalg.norm(grad_J) <= self.tol:
                ...
                self.logger[
                    'exit_message'
                ] = f'Model.__fit_gd: Convergence achieved in {iter_count} iterations.'
                break
            ...
            self.logger[
                'exit_message'
            ] = f'Model.__fit_gd: Maximum number of {self.maxiter} iterations achieved.'
...
def polynomial_model(params):
    ...
    default_params = dict(
        d=2, maxiter=int(1e5), solver='gd', lr=1e-2, div_tol=1e3
    )
    ...
    model = make_pipeline(
        ...
        Model(
            lamb=lamb, maxiter=maxiter, solver=solver, lr=lr, div_tol=div_tol
        ),
    )
...
pipe = manager.provide_model(
    ...
    params=dict(d=d, lamb=lamb, solver='gd', lr=lr, div_tol=1e3, fit=False),
    load=False,
    save=False,
)
    ...
    model = make_pipeline(
        ...
        Model(
            **utils.extract_values(
                params, ('lamb', 'maxiter', 'solver', 'lr', 'div_tol')
            )
        ),
    )
...
pipe = manager.provide_model(
    creator_method=model_creator,
    params=dict(d=1, lamb=lamb, solver='gd', lr=lr, div_tol=1e3, fit=False),
    load=True,
    save=True,
)
...
pipe = manager.provide_model(
    creator_method=model_creator,
    params=dict(d=2, lamb=lamb, solver='gd', lr=lr, div_tol=1e3, fit=False),
    load=True,
    save=True,
)
...
pipe = manager.provide_model(
    creator_method=model_creator,
    params=dict(d=3, lamb=lamb, solver='gd', lr=lr, div_tol=1e3, fit=False),
    load=True,
    save=True,
)
